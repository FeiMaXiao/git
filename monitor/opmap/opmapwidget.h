...
public:
	...
	MapGraphicItem *getMapItem();
	void zoomInMap(int nScale = 5);						 							//设置缩放级别 lianglongfei add 2016.9.9
...
signals:
	...
	void sig_mouseLeftPress(QMouseEvent event);
	void sig_mouseLeftRelease(QMouseEvent event);
	void sig_mouseRightPress(QMouseEvent event);
	void sig_mouseRightRelease(QMouseEvent event);
	void sig_mouseMove(QMouseEvent event);
	void sig_wpMouseRightClicked(int num);
    /**
...
public slots:
	...
	void slot_wpMouseRightClicked(WayPointItem *item);
};
}
#endif // OPMAPWIDGET_H
